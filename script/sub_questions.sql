...
(
    ...
    PRIMARY KEY (`id`),
    KEY `sub_question_main_id_key` (`main_id`),
    CONSTRAINT `sub_question_main_id_key` FOREIGN KEY (`main_id`) REFERENCES `main_question` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE = InnoDB
  AUTO_INCREMENT = 106
...
/*!40000 ALTER TABLE `sub_question`
    DISABLE KEYS */;
INSERT INTO `sub_question`
VALUES (4, 125, '查询Tom和Jerry的存款', 'select `name`,`amount` from blank_data where name=\'Tom\' or name=\'Jerry\';',
        '2020-11-16 07:17:06', '2020-12-28 17:33:56', 1, 0),
       (5, 125, '查询Tom和Jerry的地址', 'select `name`,`address` from address_data where name=\'Tom\' or name=\'Jerry\';',
        '2020-11-16 07:18:08', '2020-12-30 10:40:34', 1, 0),
       (10, 1, '编写一个sql语句，查询累计工作时间超过1000的职工，结果返回职工工号eno。',
        'select w.eno \nfrom works w\ngroup by w.eno\nhaving sum(hours) > 1000;', '2020-11-25 12:49:46',
        '2021-04-10 20:10:41', 1, 0),
       (11, 1, '编写一个sql语句，查询没有登记家属的职工，结果返回职工工号eno。',
        'select e.eno\nfrom employees e\nwhere not exists (\n	select *\n	from relations r\n	where r.eno = e.eno\n);',
        '2020-11-25 12:50:18', '2021-04-10 20:10:36', 1, 0),
       (12, 1, '编写一个sql语句，查询所有职工都参与的项目，结果返回项目编号pno。',
        'select p.pno\nfrom projects p\nwhere not exists (\n    select * \n    from employees e\n    where not exists(\n        select *\n        from works w\n        where w.eno = e.eno and w.pno = p.pno\n    )\n)',
        '2020-11-25 12:51:08', '2021-03-30 21:59:30', 1, 0),
       (13, 1, '编写一个sql语句，查询每一个部门中薪水最高的职工，结果返回部门编号dno，薪水最高的职工工号eno和薪水salary，以dno升序排列。',
        'select e1.dno,e1.eno, e1.salary\nfrom employees e1\nwhere e1.salary in (\n	select MAX(salary)\n	from employees e2\n	where e1.dno = e2.dno\n	group by dno\n)\norder by dno;',
        '2020-11-25 12:53:41', '2021-03-30 22:01:33', 2, 1),
       (14, 2, '编写一个sql语句，找出年龄在35以上的并且在2020-09-01至2020-09-30期间没有预定红色（RED）船只的水手，结果返回水手姓名sname。',
        'select s.sname from sailors s\nwhere s.age > 35 and not exists (\n	select * \n	from reserves r join boats b\n	on r.bid = b.bid \n	where  \n	r.sid = s.sid and \n	b.color = \'RED\' and\n	(r.reserve_date between \'2020-09-01\' and \'2020-09-30\')\n);',
        '2020-11-25 14:42:26', '2021-04-07 11:13:59', 1, 0),
       (15, 3, '编写一个sql语句，查询每件商品都购买过的用户，结果返回用户编号customer_id。',
        'select c.customer_id\nfrom customers c\nwhere not exists (\n    select * \n    from products p\n    where not exists (\n        select * \n        from orders o\n        where o.customer_id = c.customer_id and o.product_id = p.product_id\n    )\n);',
        '2020-11-25 14:59:19', '2021-03-30 22:11:51', 1, 0),
       (16, 3,
        ...
        '2020-11-25 15:01:38', '2021-04-06 23:58:13', 2, 0),
       (17, 3, '编写一个sql语句，找出每件商品的最新订单(可能有多个)，结果返回商品名称product_name，订单号order_id和订单日期order_date，以product_name升序排列。',
        'SELECT\n	p.product_name AS product_name,\n	o.order_id,\n	o.order_date\nFROM\n	orders o,\n	customers c,\n	products p\nWHERE\n	o.customer_id = c.customer_id\nAND o.product_id = p.product_id\nAND (o.product_id, o.order_date) IN (\n	SELECT\n		product_id,\n		MAX(order_date)\n	FROM\n		orders\n	GROUP BY\n		product_id\n)\nORDER BY product_name;\n',
        '2020-11-25 15:10:49', '2021-03-30 22:07:24', 2, 1),
       (18, 3,
        ...
        '2020-11-25 15:14:53', '2021-03-30 22:11:35', 2, 1),
       (19, 4, '编写一个sql语句，查询参加比赛场次最多的选手，若参与比赛场次相同，选择用户编号player_id较小的选手，结果返回用户编号player_id和参与的比赛数量match_num。',
        ...
        '2020-11-25 15:28:48', '2021-03-30 22:13:15', 2, 0),
       (20, 4, '编写一个sql语句，查询各组进行的比赛场次，结果返回组号group_id及各组的场次数match_num。',
        'select group_id, count(match_id) as match_num\nfrom matches\ngroup by group_id;', '2020-11-25 15:31:30',
        '2021-03-30 22:13:39', 1, 0),
       (21, 4, '编写一个sql语句，查询分差最大的一场比赛，若分差相同，选择编号match_id较小的比赛，结果返回比赛编号match_id和分差sub（大于0）。',
        'select match_id, ABS(first_score - second_score) as sub\nfrom matches\norder by ABS(first_score - second_score) desc, match_id asc\nlimit 1;',
        '2020-11-25 15:37:23', '2021-03-30 22:14:39', 2, 0),
       (22, 4,
        ...
        '2020-11-25 15:39:59', '2021-03-30 22:29:39', 3, 1),
       (23, 5, '编写一个sql查询，查找订单数量最多的一天以及当天的订单数量，订单数相同时选择日期最小的一天，结果返回日期order_date和订单数量num。',
        'select order_date,count(*) as num\nfrom orders \ngroup by order_date\norder by num DESC,order_date asc\nlimit 1;',
        '2020-11-25 15:46:15', '2021-03-30 22:30:34', 1, 0),
       (24, 5, '编写一个sql语句，查找买到过自己最喜欢的商品的用户，结果返回用户编号user_id。',
        'select u.user_id \nfrom users u\nwhere exists (\n	select * \n	from orders o LEFT JOIN items i\n	on o.item_id = i.item_id\n	where o.buyer_id = u.user_id and u.favorite_brand = i.item_brand\n);',
        '2020-11-25 15:47:57', '2021-03-30 22:30:59', 2, 0),
       (25, 5, '编写一个sql语句，查询每个用户的注册日期以及在2019年作为买家的订单总数，结果返回用户编号user_id，注册日期join_date和订单数量orders_in_2019，以user_id升序排列。',
        'select user_id, join_date, count(order_id) orders_in_2019\nfrom users left join orders\non user_id = buyer_id and year(order_date)=\'2019\'\ngroup by user_id\norder by user_id;',
        '2020-11-25 15:50:21', '2021-04-07 11:54:25', 2, 1),
       (26, 5,
        ...
        '2020-11-25 16:15:32', '2021-04-14 00:12:51', 3, 1),
       (27, 6, '编写一个sql语句，求出好友申请的总通过率accept_rate，用2位小数表示。通过率计算公式为接受好友申请的数目除以申请总数（申请和接收可能会有重复，此时均视作一次）。',
        ...
        '2020-11-25 16:29:45', '2021-04-10 21:29:14', 2, 0),
       (28, 6, '编写一个sql语句，查询发出过申请，但所有申请都未通过的用户，结果返回用户编号user_id。',
        ...
        '2020-11-25 16:30:33', '2021-03-30 22:47:34', 2, 0),
       (29, 6, '编写一个sql语句，查询每个月的通过率，结果返回月份mon及通过率accept_rate（不包括通过率为0的月份），以mon升序排列。',
        ...
        '2020-11-25 16:34:38', '2021-03-30 22:48:20', 2, 1),
       (30, 6, '编写一个sql语句，找出拥有最多的好友的用户以及他拥有的好友数目，好友数相同时选择user_id较小的用户，结果返回用户编号user_id和好友数目friend_num。',
        ...
        '2020-11-25 16:36:42', '2021-03-30 22:48:56', 2, 0),
       (31, 7, '编写一个sql语句，查询所有浏览过自己文章的作者，结果返回用户编号id，以id升序排列。',
        'SELECT distinct author_id as id\nFROM views\nWHERE author_id = viewer_id\nORDER BY author_id;',
        '2020-11-25 16:44:02', '2021-03-30 22:52:20', 1, 1),
       (32, 7, '编写一个sql语句，找出曾在一天内阅读至少两篇文章的人，结果返回用户编号viewer_id，以viewer_id升序排列。',
        'select  distinct viewer_id from views \ngroup by viewer_id,view_date\nhaving count(distinct article_id) >= 2\norder by viewer_id;',
        '2020-11-25 16:45:09', '2021-03-30 22:53:20', 2, 1),
       (33, 7, '编写一个sql语句，找出阅读文章总数最多的用户，阅读数量一样时选择view_id较小的用户，结果返回用户编号viewer_id和阅读文章数量article_num。',
        'select viewer_id , count(*) as article_num  from views \ngroup by viewer_id \norder by article_num desc, viewer_id asc\nlimit 1;',
        '2020-11-25 16:47:33', '2021-04-07 00:20:29', 2, 0),
       (34, 7, '编写一个sql语句，找出2020-08-01至2020-08-31期间阅读过3号作者文章的人，结果返回用户编号id，以id升序排列。',
        'select distinct viewer_id as id\nfrom views v1  \nwhere exists(\n	select *\n	from views v2\n	where v2.viewer_id = v1.viewer_id and v2.author_id = 3\n				and (v2.view_date BETWEEN \'2020-08-01\' and \'2020-08-31\')\n)\norder by id;',
        '2020-11-25 16:48:57', '2021-04-07 11:16:15', 2, 1),
       (35, 8, '编写一个sql语句，查询每个用户最近一天登录的日子，结果返回用户编号user_id和登录日期date，以user_id升序排列。',
        'SELECT user_id, MAX(login_date) as date FROM logins\nGROUP BY user_id\nORDER BY user_id;',
        '2020-11-25 18:08:13', '2021-03-30 22:54:43', 1, 1),
       (36, 8, '编写一个sql语句，查询新登录用户的留存率，即新用户第1天登陆之后，第2天再次登陆的概率，结果返回留存率rate。',
        ...
        '2020-11-25 18:09:23', '2021-04-12 11:04:12', 3, 0),
       (37, 8, '编写一个sql语句，查询登录新用户个数不少于2个的日期，结果返回日期login_date和登录新用户个数new_user_num，以login_date升序排序。',
        ...
        '2020-11-25 18:11:07', '2021-03-30 22:55:57', 2, 1),
       (38, 8, '编写一个sql语句，查询每个日期新用户次日留存率（包括留存率为0的日期），即该日登录的新用户第二日仍然登录的概率，返回日期date和留存率rate，保留小数点后3位，以date升序排列。',
        ...
        '2020-11-25 18:16:53', '2021-03-30 22:56:49', 3, 1),
       (39, 9, '编写一个sql语句，查询各个岗位分数的平均数，结果返回岗位名称job以及平均成绩avg，保留小数点后3位，以avg降序排序。',
        'select job,round(sum(score)*1.0/count(user_id),3) as avg from grades\ngroup by job order by avg desc;',
        '2020-11-25 18:51:53', '2021-03-30 22:57:28', 1, 1),
       (40, 9, '编写一个sql语句，查询分数大于其所在岗位平均分的用户，结果返回用户编号user_id，岗位名称job和该用户分数score，以user_id升序排序。',
        'select grades.* from grades join \n(select job,round(sum(score)*1.0/count(user_id),3) as avg from grades\ngroup by job) as t\non grades.job=t.job\nwhere grades.score > t.avg\norder by user_id;',
        '2020-11-25 18:52:57', '2021-03-30 22:58:35', 2, 1),
       (41, 9,
        ...
        '2020-11-25 18:55:23', '2021-04-07 00:07:37', 1, 1),
       (42, 9, '编写一个sql语句，查询分数位于各个岗位中位数范围内的用户信息，结果返回用户编号user_id，岗位名称job，用户分数score和排名t_rank，以user_id升序排列。',
        ...
        '2020-11-25 18:56:14', '2021-03-30 23:04:04', 3, 1),
       (43, 10, '编写一个sql语句，查找Technology部门工资的平均值，结果返回部门名称department和平均工资avg_salary。',
        'select d.department_name as department, AVG(e.salary) as avg_salary\nfrom employees e LEFT JOIN departments d\non e.department_id = d.department_id\nwhere d.department_name = \'Technology\'\ngroup by e.department_id;',
        '2020-11-25 19:11:07', '2021-03-30 23:04:25', 1, 0),
       (44, 10, '编写一个sql语句，查找各部门最高工资与最低工资的差值，结果返回部门名称department和差值sub。',
        'select d.department_name department, MAX(salary)-MIN(salary) as sub\nfrom employees e LEFT JOIN departments d \non e.department_id = d.department_id\ngroup by e.department_id;',
        '2020-11-25 19:12:40', '2021-03-30 23:07:16', 2, 0),
       (45, 10, '编写一个sql语句，获取各个部门第二高的薪水，结果返回部门名称department，员工姓名name和工资salary，以department升序排列。',
        ...
        '2020-11-25 19:17:46', '2021-03-30 23:04:59', 3, 1),
       (46, 10, '编写一个sql语句，找出每个部门获得前三高工资的所有员工，结果返回部门名称department，员工姓名name和工资salary，以department升序，salary降序排列。',
        ...
        '2020-11-25 19:20:08', '2021-03-30 23:07:09', 3, 1),
       (47, 2, '编写一个sql语句，找出预定了所有船的水手，结果返回水手姓名sname。',
        'select s.sname from sailors s\nwhere not exists (\n	select * from boats b \n	where not exists (\n		select * from reserves r\n		where r.sid = s.sid and r.bid = b.bid\n	)\n);',
        '2020-11-25 20:03:45', '2021-03-30 22:02:23', 1, 0),
       (48, 2, '编写一个sql语句，找出2020-05-01至2020-05-31期间预定过绿色船（GREEN）的等级最高的水手，结果返回水手姓名sname。',
        'select s.sname\nfrom sailors s\nwhere exists(\n	select *\n	from reserves r join boats b\n	on r.bid = b.bid\n	where b.color = \'GREEN\' and r.sid = s.sid and r.reserve_date between \'2020-05-01\' and \'2020-05-31\'\n)\norder by rating DESC\nlimit 1;',
        '2020-11-25 20:04:04', '2021-04-07 11:18:28', 2, 0),
       (49, 2, '编写一个sql语句，找出年龄在35岁以上，并且在2020-08-01至2020-08-31期间同时预定了红色船（RED）和绿色船（GREEN）的水手，结果返回水手姓名sname。',
        ...
        '2020-11-25 20:10:28', '2021-04-10 20:16:19', 2, 0),
       (52, 11, '查询那些既没有最多，也没有最少参与者的活动的名字，可以以任何顺序返回结果，activities 表的每项活动的参与者都来自friends 表。结果字段:activity ',
        'select activity\nfrom friends\ngroup by activity\nhaving count(*)>any(\n    select count(*) from friends group by activity\n) and count(*)<any(\n    select count(*) from friends group by activity\n)',
        '2020-11-26 11:28:29', '2021-04-13 21:34:13', 3, 0),
       (53, 11, '查询在2月20~2月28之间能进行的活动及参加该活动的朋友名字。结果字段包含name，activity。',
        ...
        '2020-11-26 11:29:13', '2020-12-30 11:01:55', 2, 0),
       (55, 11, '查询进行eating的用户有哪些。结果字段：name',
        'select friends.name \nfrom friends,activities\nwhere friends.activity=activities.activity and friends.activity=\'eating\'',
        '2020-11-26 11:30:52', '2021-04-13 21:36:47', 1, 0),
       (56, 12, '查找在 2020 年 2 月 平均评分最高 的电影名称。 如果有相同的，返回字典序较小的电影名称。结果字段：title',
        'select title from \n(select m.title,avg(mr.rating) as rt from movie_rating mr\ninner join movies m\non m.movie_id = mr.movie_id \ninner join users u \non u.user_id = mr.user_id \nwhere mr.created_at between \'2020-02-01\' and \'2020-02-29\'\ngroup by m.movie_id\norder by rt desc,m.title asc\nlimit 1) b',
        '2020-11-26 11:33:33', '2021-04-10 21:21:42', 3, 1),
       (57, 12, '查找评论电影数量最多的用户名。 如果出现平局，返回字典序较小的用户名。结果字段：name',
        '(SELECT name\nFROM users AS U\nINNER JOIN movie_rating AS MR\nON U.user_id = MR.user_id\nGROUP BY U.user_id\nORDER BY COUNT(*) DESC, name\nLIMIT 1)\n',
        '2020-11-26 11:34:06', '2021-04-10 12:05:40', 3, 1),
       (58, 12, '求每部电影的最高分，最低分以及平均分。结果字段包含movie_id, title, avg_rating, max_rating, min_rating。',
        ...
        '2020-11-26 11:34:36', '2021-04-12 10:35:12', 2, 0),
       (59, 12, '查询一月份评论过电影的用户。结果集包含，name , title, created_at。',
        ...
        '2020-11-26 11:35:06', '2020-12-30 11:03:00', 2, 0),
       (60, 12, '查找用户id为1的用户看过的电影以及所给的评分。结果字段包含user_id，name，title，rating。',
        ...
        '2020-11-26 11:35:30', '2021-04-10 20:55:55', 1, 0),
       (61, 13, '查询金额最大的发票所对应的发票号，用户ID，用户姓名。结果字段：invoice_id，price, user_id，customer_name',
        ...
        '2020-11-26 11:50:08', '2021-04-10 21:09:40', 2, 0),
       (62, 13, '求拥有发票数大于1的用户的联系人名字及电子邮件。结果字段：user_id，contact_name，contact_email',
        ...
        '2020-11-26 11:50:58', '2021-04-10 21:09:52', 2, 0),
       (63, 13, '求拥有一张发票的顾客及其电子邮箱。结果字段包含：user_id，customer_name，email',
        'select t.user_id,customer_name,email\nfrom\n(\n    SELECT user_id,count(*) count_temp\n    FROM invoices\n    GROUP BY user_id  \n    HAVING count_temp=1\n)t, customers\nwhere t.user_id=customers.customer_id',
        '2020-11-26 11:51:26', '2021-04-10 21:10:06', 2, 0),
       (64, 13, '求拥有联系人的顾客的id和姓名以及联系人姓名。结果字段customer_id，customer_name，contact_name',
        'select customer_id,customer_name,contact_name\nfrom customers,contacts\nwhere customer_id=user_id',
        '2020-11-26 11:52:08', '2021-04-10 21:23:18', 1, 0),
       (65, 14, '每组的获胜者是在组内累积得分最高的选手。若平局player_id 最小选手获胜。查询每组中的获胜者。结果字段包含group_id,player_id。',
        ...
        '2020-11-26 11:56:17', '2020-12-30 23:40:34', 3, 0),
       (66, 14, '查询姓名为Aron的参赛者参加过的训练赛赛事分数。结果字段包含player_name ,match_id, score（不能为0）。 ',
        ...
        '2020-11-26 11:57:32', '2020-12-30 23:33:13', 2, 0),
       (67, 14, '根据Scores表查询性别为‘F’在每一天的总分，并按性别和日期对结果排序。结果字段包含gender, day, total。',
        'select s1.gender,s1.day,sum(s2.score_points) as total from scores s1\ninner join scores s2\non s1.gender = s2.gender \nand s1.day >= s2.day \nand s1.gender=\'F\'\ngroup by s1.gender,s1.day  \norder by s1.gender,s1.day',
        '2020-11-26 11:58:53', '2020-12-30 23:33:27', 2, 1),
       (68, 14, '查询在1月进行训练赛的参赛者姓名，性别，分数。结果字段包含player_name ,gender, score_points。',
        'select player_name,gender,score_points\nfrom scores\nwhere day between \'2020-01-01\' and \'2020-01-31\'',
        '2020-11-26 11:59:21', '2020-12-30 23:33:32', 1, 0),
       (69, 14, '查找在scores表中获得最低分数的球员的姓名以及他的id。结果字段包含player_name, player_id，score_points.',
        ...
        '2020-11-26 11:59:43', '2020-12-31 00:16:14', 2, 0),
       (70, 15,
        ...
        '2020-11-26 14:14:53', '2021-04-10 12:03:32', 3, 1),
       (71, 15,
        ...
        '2020-11-26 14:16:57', '2021-04-08 23:53:08', 3, 1),
       (72, 15,
        ...
        '2020-11-26 14:20:11', '2021-04-08 23:53:30', 2, 1),
       (73, 15, '查询在 2020 年 8 月份被卖出的产品名字。结果字段： product_name',
        'SELECT product_name\nFROM products JOIN orders USING (product_id)\nWHERE order_date LIKE \"2020-08%\"\nGROUP BY product_name',
        '2020-11-26 14:20:57', '2021-04-08 23:53:39', 1, 0),
       (74, 15, '查询购买了产品ID 为1和产品 ID为2却没有购买产品ID为3的顾客的 ID 和姓名。结果字段： customer_id, name',
        'SELECT\n    customer_id, name\nFROM\n    customers\nWHERE\n    customer_id NOT IN (\n        SELECT customer_id\n        FROM orders\n        WHERE product_id =3\n    ) AND customer_id IN (\n        SELECT customer_id\n        FROM orders\n        WHERE product_id =2\n    ) AND customer_id IN (\n        SELECT customer_id\n        FROM orders\n        WHERE product_id =1\n    )\nORDER BY customer_id',
        '2020-11-26 14:21:30', '2021-04-08 23:53:51', 2, 0),
       (75, 16, '找出与”赵正义”在同一天借书的读者姓名、所在单位及借书日期 。结果字段： name,company',
        ...
        '2020-11-26 14:24:24', '2021-04-08 23:54:25', 3, 0),
       (76, 16, '查询财会系2006年7月以后没有借书的读者借书证号、姓名及单位。结果字段：reader_id,name,company',
        ...
        '2020-11-26 14:24:45', '2021-04-08 23:54:38', 1, 0),
       (77, 16, '找出当前至少借阅了2本图书(大于等于2本)的读者姓名及其所在单位。结果字段：name,company',
        ...
        '2020-11-26 14:26:19', '2021-04-08 23:54:51', 2, 0),
       (78, 16, '题目：找出作者为李某的书籍的被借阅情况。结果字段：writer, book_name, borrow_date。',
        ...
        '2020-11-26 14:26:37', '2021-04-10 12:02:58', 2, 0),
       (79, 16, '查找“高等教育出版社”的所有图书名称(BOOK_NAME)及单价(PRICE)，结果按单价降序排序。结果字段：book_name,price',
        'SELECT DISTINCT(book_name),price FROM books\n    WHERE output=\'高等教育出版社\' \n    ORDER BY price DESC;',
        '2020-11-26 14:26:56', '2021-04-08 23:55:17', 1, 1),
       (80, 17,
        ...
        '2020-11-26 14:30:44', '2021-04-13 21:43:01', 3, 1),
       (81, 17, '查询所有活跃的业务。结果字段只包含business_id。',
        'SELECT business_id\nFROM events AS e\nJOIN (\n    SELECT event_type, AVG(occurences) AS eventAvg\n    FROM events\n    GROUP BY event_type\n) AS e1 \nON e.event_type = e1.event_type\nWHERE e.occurences > e1.eventAvg\nGROUP BY business_id\nHAVING COUNT(*) >= 2',
        '2020-11-26 14:33:08', '2021-03-05 10:37:36', 3, 0),
       (82, 17, '分别求出每个event事件的成功天数与失败天数。结果字段包含：event_type，success_count，fail_count。',
        ...
        '2020-11-26 14:34:14', '2021-04-13 21:55:20', 3, 0),
       (83, 17, ' 求出reviews事件在18年12月份系统里的最小的失败次数。结果字段：occurences，event_type，fail_count',
        ...
        '2020-11-26 14:37:17', '2021-04-13 21:55:33', 2, 0),
       (84, 17, '求一下事件类型发生次数最多的商业ID。结果字段仅包含business_id。',
        'select business_id\nfrom events\nwhere occurences=(select max(occurences)\n                   from events)\n',
        '2020-11-26 14:37:50', '2020-12-30 11:10:23', 1, 0),
       (85, 18, '查询没有学全所有课程的同学的信息。结果字段包含s_id, s_name, s_sex, s_birthday。',
        ...
        '2020-11-26 14:55:25', '2021-04-08 23:57:26', 1, 0),
       (89, 19, '查询每个部门工资最高的员工，结果字段包含：department_name,employee,salary',
        'SELECT\n	department.NAME AS department_name,\n	employee.NAME AS employee,\n	salary \nFROM\n	employee,\n	department \nWHERE\n	employee.DepartmentId = department.Id \n	AND ( employee.DepartmentId, Salary ) \n    IN (SELECT DepartmentId, max( Salary ) \n        FROM employee \n        GROUP BY DepartmentId )',
        '2020-11-26 15:21:26', '2021-04-09 00:03:08', 2, 0),
       (90, 19, '查询收入超过他们经理的员工的姓名。结果字段:employee',
        'SELECT\n    a.Name AS \'employee\'\nFROM\n    employee AS a,\n    employee AS b\nWHERE\n    a.ManagerId = b.Id\n        AND a.Salary > b.Salary\n;',
        '2020-11-26 15:21:56', '2021-04-09 00:03:23', 1, 0),
       (91, 19, '查询职员表中第二高的薪水（注：若AB薪资都是3000，C薪资2000，则第二高指的是C的2000）。结果字段：SecondHighestSalary',
        'SELECT\n    (SELECT DISTINCT\n            salary\n        FROM\n            employee\n        ORDER BY Salary DESC\n        LIMIT 1 OFFSET 1) AS SecondHighestSalary\n;',
        '2020-11-26 15:22:50', '2021-04-09 00:03:51', 2, 0),
       (92, 20, '查询工资高于部门号为30的所有员工工资水平的员工信息。 结果字段：empno，ename，job，mgr，hiredate，sal，comm，deptno。',
        'select * from emp \nwhere sal > ( select max(sal) from emp where deptno = 30);  ', '2020-11-26 15:25:04',
        '2021-04-13 22:01:29', 2, 0),
       (93, 20, '返回工资处于第四级别的员工的姓名。结果字段包含：ename，sal。',
        'select ename,sal from emp e ,salgrade s \nwhere e.sal >= s.losal and e.sal <= s.hisal \nand s.grade = 4; ',
        '2020-11-26 15:25:24', '2021-04-13 22:04:54', 1, 0),
       (94, 20, '返回工资为二等级的职员名字、部门名称、工资和二等级的最低工资和最高工资 。结果字段： ename，dname，sal，losal，hisal。',
        ...
        '2020-11-26 15:25:42', '2021-04-13 22:12:56', 2, 0),
       (95, 20, '返回工资等级高于smith且直属领导编号为7298的员工信息。结果字段：empno，ename，job，mgr，hiredate，sal，comm，deptno。',
        ...
        '2020-11-26 15:25:57', '2021-04-13 22:03:03', 2, 0),
       (96, 20, '计算出部门号为20的员工的年薪，并且对年薪进行升序排序。结果字段包含ename，deptno，ySalary。',
        'select ename, deptno,sal * 12 as ySalary \nfrom emp \nwhere deptno=20\norder by ySalary; ',
        '2020-11-26 15:26:24', '2021-04-13 22:05:08', 2, 1),
       (98, 18, '查询课程号为1和2的成绩在第2名到第3名的学生信息及该课程成绩。结果字段包含：s_id, s_name, s_birthday, s_sex, rank, s_score, c_id',
        ...
        '2020-11-26 19:15:55', '2020-12-30 11:11:19', 3, 0),
       (99, 18,
        ...
        '2020-11-26 19:18:06', '2021-04-14 11:16:16', 3, 0),
       (100, 18,
        '统计各科成绩各分数段人数：课程编号,课程名称,[100-85],[85-70],[70-60],[0-60]及所占百分比。结果字段：c_name, c_id，85-100, percent, 70-80, percent, 60-70, percent, 0-60, percent。',
        ...
        '2020-11-26 19:18:45', '2020-12-30 11:11:34', 3, 0),
       (101, 18, '查询没学过张三老师讲授的任一门课程的学生姓名。结果字段：s_name。',
        ...
        '2020-11-26 19:22:51', '2021-04-08 23:58:39', 1, 0),
       (103, 13,
        ...
        '2020-12-30 23:16:36', '2021-04-12 10:56:02', 3, 1),
       (104, 19, '编写SQL查询来查找每个部门的薪水中位数。若某一部门薪水情况是：1000,2000,3000,4000，则中位数为2000及3000.结果字段：departmentId, salary',
        ...
        '2021-01-02 16:06:41', '2021-04-09 00:04:39', 3, 0),
       (105, 11, '编写sql语句查询进行相同活动的用户的数量，要求结果字段有cnt（cnt表示用户的数量，依据题意至少为2），activity。结果字段：cnt，activity',
